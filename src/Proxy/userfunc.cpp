...
const struct user_command_type user_commands[] = {
    {"maddroom", usercmd_maddroom, 0, USERCMD_FLAG_REDRAW, "Add current room to database [READ HELP]",
     "    Usage: maddroom \r\n"
     "    Examples: maddroom \r\n\r\n"
    ...
    {"mhelp", usercmd_mhelp, 0, 0, "Brief help files or commands overview.",
    ...
    {"mmap", usercmd_config, USER_CONF_MAP, USERCMD_FLAG_SYNC, "Turn mapping mode on/off, if possible.",
     "    Usage: mmap [boolean]\r\n"
    ...
    {"mbrief", usercmd_config, USER_CONF_BRIEF, 0, "Turn mappers built in brief mode on/off.",
    ...
    {"mcheckexits", usercmd_config, USER_CONF_EXITS, 0, "Turn exits analyzer on/off.",
     ...
     "map.\r\n"},
    {"mautomerge", usercmd_config, USER_CONF_AUTOMERGE, 0, "Turn descriptions analyzer in mapping mode on/off.",
    ...
    {"mangrylinker", usercmd_config, USER_CONF_ANGRYLINKER, 0, "Turn the AngryLinker on/off.",
    ...
    {"mduallinker", usercmd_config, USER_CONF_DUALLINKER, 0, "Turn the DualLinker on/off.",
    ...
    {"mcheckterrain", usercmd_config, USER_CONF_TERRAIN, 0, "Turn terrain analyzer on/off.",
    ...
    {"msave", usercmd_msave, 0, 0, "Save/Save as current database.",
    ...
    {"mload", usercmd_mload, 0, USERCMD_FLAG_REDRAW, "Load file/Reload the database from disk.",
    ...
    {"mreset", usercmd_mreset, 0, USERCMD_FLAG_REDRAW, "Reset mappers state stacks.",
     "    Usage: mreset\r\n"
     "    Examples: mreset\r\n\r\n"
    ...
    {"mstat", usercmd_mstat, 0, 0, "Display settings and mappers state stacks.",
     "    Usage: mstat\r\n"
     "    Examples: mstat\r\n\r\n"
     "    This command displays settings, stacks and possible current position room id's\r\n"},
    {"minfo", usercmd_minfo, 0, 0, "Display current rooms data (or by given id).",
     "    Usage: minfo [id]\r\n"
    ...
    {"mmerge", usercmd_mmerge, 0, USERCMD_FLAG_SYNC | USERCMD_FLAG_REDRAW, "Merge twin rooms - manual launch.",
    ...
    {"mdecx", usercmd_mdec, USER_DEC_X, USERCMD_FLAG_SYNC | USERCMD_FLAG_REDRAW, "Decrease the X coordinate.",
     "    Usage: mdecx [integer]\r\n"
     "    Examples: mdecx / mdecx 2\r\n\r\n"
     "mdecx substracts 1 from the X coordinate. If you give an integer argument X, X will be substracted.\r\n"},
    {"mincx", usercmd_mdec, USER_INC_X, USERCMD_FLAG_SYNC | USERCMD_FLAG_REDRAW, "Increase the X coordinate.",
     "    Usage: mincx [integer]\r\n"
     "    Examples: mincx / mincx 2\r\n\r\n"
     "mdecx adds 1 to the X coordinate. If you give an integer argument X, X will be added.\r\n"},
    {"mdecy", usercmd_mdec, USER_DEC_Y, USERCMD_FLAG_SYNC | USERCMD_FLAG_REDRAW, "Decrease the Y coordinate.",
     "    See mdecx.\r\n"},
    {"mincy", usercmd_mdec, USER_INC_Y, USERCMD_FLAG_SYNC | USERCMD_FLAG_REDRAW, "Increase the Y coordinate.",
     "    See mdecx.\r\n"},
    {"mdecz", usercmd_mdec, USER_DEC_Z, USERCMD_FLAG_SYNC | USERCMD_FLAG_REDRAW, "Decrease the Z coordinate.",
     "    See mdecx.\r\n"},
    {"mincz", usercmd_mdec, USER_INC_Z, USERCMD_FLAG_SYNC | USERCMD_FLAG_REDRAW, "Increase the Z coordinate.",
     "    See mdecx.\r\n"},
    {"mcoord", usercmd_mcoord, 0, USERCMD_FLAG_SYNC | USERCMD_FLAG_REDRAW, "Set the coordinates for current room.",
     "    Usage: mcoord [X] [Y] [Z]\r\n"
     "    Examples: mcoord -2 4 5 / mcoord -2 / mcoord -2 4 / mcoord -2 4 5 \r\n\r\n"
     "Set the coordinates of the current room to the given values.\r\n"},

    {"mdoor", usercmd_mdoor, USER_DOOR_NORMAL, USERCMD_FLAG_SYNC | USERCMD_FLAG_REDRAW, "Add a door in direction",
    ...
    {"mnote", usercmd_mnote, 0, USERCMD_FLAG_SYNC | USERCMD_FLAG_REDRAW, "Add a note to the room you are standing in",
     "   Usage: mnote <note>\r\n"
    ...
    {"mexit", usercmd_mdoor, USER_DOOR_EXIT, USERCMD_FLAG_SYNC | USERCMD_FLAG_REDRAW, "Add a nonsecret door",
    ...
    {"mmark", usercmd_mmark, 0, USERCMD_FLAG_SYNC | USERCMD_FLAG_REDRAW, "Mark/Flag some direction.",
     "    Usage: mmark <dir> <flag>\r\n"
     "    Examples: mmark west undefined / mmark w death\r\n\r\n"
     "    Possible flags: undefined, death\r\n"},
    {"mlink", usercmd_mlink, 0, USERCMD_FLAG_SYNC | USERCMD_FLAG_REDRAW, "Link current room with some other.",
     ...
     "    Link two rooms.\r\n"},
    {"mdetach", usercmd_mdetach, 0, USERCMD_FLAG_SYNC | USERCMD_FLAG_REDRAW, "Detach connections in 2 rooms.",
    ...
    {"mgoto", usercmd_mgoto, 0, USERCMD_FLAG_REDRAW, "Set some room as current (using id or direction).",
    ...
    {"maction", usercmd_maction, 0, USERCMD_FLAG_INSTANT, "Perform some action with doors in current room.",
    ...
    {"mdebug", usercmd_mdebug, 0, 0, "Configure debug information/messages.",
...
void display_debug_settings()
{
    ...
    for (i = 0; debug_data[i].name; i++)
        send_to_user("   %-14s %-50s [%-3s]\r\n", debug_data[i].name, debug_data[i].desc, ON_OFF(debug_data[i].state));
...
USERCMD(usercmd_maddroom)
{
    ...
    if (line) {
    } /* just to avoid warning about unused variable - its annoying */
...
#define MACTION_SEND_DOOR(door, dir)                                                                                   \
    ...
    const char *short_exits[] = {"n", "e", "s", "w", "u", "d"};
...
USERCMD(usercmd_mregion)
{
    ...
    if (!*p) {
        ...
        send_to_user("Settings: autoReplace %s, info in client view %s, info in renderer %s\r\n",
                     ON_OFF(conf->getRegionsAutoReplace()), ON_OFF(conf->getShowRegionsInfo()),
            ...
            for (unsigned int spell = 0; spell < conf->spells.size(); spell++) {
                ...
                if (conf->spells[spell].name == timer && conf->spells[spell].addon == true) {
                    ...
                    send_to_user("--[ Timer %s stopped. Duration: %s.\r\n\r\n", (const char *)conf->spells[spell].name,
                                 qPrintable(conf->spellUpFor(spell)));
                    send_prompt();
                    return USER_PARSE_SKIP;
                }
            }
