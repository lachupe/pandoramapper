...
bool Cdispatcher::parseXml(QByteArray tag)
{
    ...
    struct
    {
        ...
        int startType;
        int endType;
    } TagTypes[] = {{"prompt", XML_START_PROMPT, XML_END_PROMPT},
    ...
    printf("XML tag name : ...%s..., params : ...%s..., EndTag %s, endAfterTag %s\r\n", (const char *)name,
           (const char *)param, ON_OFF(endTag), ON_OFF(endAfterTag));
...
void Cdispatcher::dispatchBuffer(ProxySocket &c)
{
    ...
    for (s = (unsigned char *)c.buffer; s != stop; s++) {
        //        printf("[ amount %i - m %i, s %i,  s %i, left %i ] \r\n", amount, c.mainState, c.subState, s, stop -
        //        s);
        switch (c.mainState) {
        case NORMAL:
            switch (*s) {
...
#define SEND_EVENT_TO_ENGINE                                                                                           \
    {                                                                                                                  \
        ...
        print_debug(DEBUG_DISPATCHER,                                                                                  \
                    "event payload: name_len=%i desc_len=%i exits_len=%i movement=%i dir=%s prompt_len=%i",            \
                    event.name.length(), event.desc.length(), event.exits.length(), event.movement,                    \
                    (const char *)event.dir, event.prompt.length());                                                   \
        awaitingData = false;                                                                                          \
    ...
    for (i = 0; i < amount; i++) {
        ...
        if (buffer[i].type == IS_XML) {
            ...
            if (buffer[i].xmlType == XML_START_MOVEMENT) {
                ...
                xmlState = STATE_ROOM;
                continue;
            } else if ((buffer[i].xmlType == XML_START_NAME) && (xmlState == STATE_ROOM || xmlState == STATE_NORMAL)) {
                xmlState = STATE_NAME;
                continue;
            } else if ((buffer[i].xmlType == XML_START_DESC) && (xmlState == STATE_ROOM || xmlState == STATE_NORMAL)) {
                xmlState = STATE_DESC;
                continue;
                ...
                continue;
            } else if (buffer[i].xmlType == XML_START_EXITS && (xmlState == STATE_ROOM || xmlState == STATE_NORMAL)) {
...
void Cdispatcher::checkStateChange(QByteArray line)
{
    ...
    // BASHED STATE
    static QRegularExpression bashed =
        makeWildcardRegex("* sends you sprawling with a powerful bash.", Qt::CaseSensitive);
