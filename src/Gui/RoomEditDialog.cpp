...
void RoomEditDialog::accept()
{
    ...
    r = Map.getRoom(id);
    if (r == nullptr) {
        QMessageBox::critical(this, "Room Info Edit", QString("The room with this ID does not exist anymore."));
        return;
    }

    name = lineEdit_name->text();
    if (name.length() == 0 || name.length() > 80) {
        QMessageBox::critical(this, "Room Info Edit", QString("Bad room name!"));
        return;
    }
    ...
    if (updateExitsInfo(NORTH, r) == -1)
        return;
    if (updateExitsInfo(EAST, r) == -1)
        return;
    if (updateExitsInfo(SOUTH, r) == -1)
        return;
    if (updateExitsInfo(WEST, r) == -1)
        return;
    if (updateExitsInfo(UP, r) == -1)
        return;
    if (updateExitsInfo(DOWN, r) == -1)
        return;
