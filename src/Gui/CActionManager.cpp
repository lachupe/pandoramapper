...
CActionManager::CActionManager(CMainWindow *parentWindow)
{
    ...
    deleteFullyAct->setStatusTip(
        tr("Deletes this room and everything that leads to it, including doors in other rooms"));
...
void CActionManager::bindRooms()
{
    ...
    for (dir = 0; dir <= 5; dir++)
        ...
        if (one->isExitUndefined(dir) == true && two->isExitUndefined(reversenum(dir))) {
            ...
            if ((dir == NORTH && one->getY() < two->getY()) || (dir == EAST && one->getX() < two->getX()) ||
                (dir == WEST && one->getX() > two->getX()) || (dir == SOUTH && one->getY() > two->getY()) ||
                (dir == UP && one->getZ() < two->getZ()) || (dir == DOWN && one->getZ() > two->getZ())) {
                fits = true;
            }
    ...
    QMessageBox::critical(
        parent, "Failure",
        QString("No fitting exits found. Rooms are badly positioned or exits are not marked as Undefined."));
}
...
{
    QMessageBox::about(
        parent, tr("About Pandora"),
        tr("<h2>Pandora MUME Mapper</h2>"
#ifdef SVN_REVISION
           "<p>SVN revision ") +
            QString::number(SVN_REVISION) +
            tr(
#endif
                ...
                "<p>Visit <a href=\"http://code.google.com/p/pandoramapper/\">code.google.com/p/pandoramapper/</a> for "
                "more information."));
}
...
{
    QString s = QFileDialog::getOpenFileName(parent, "Choose a database", "database/", "XML files (*.xml)");
...
{
    QString s = QFileDialog::getSaveFileName(parent, "Choose a filename to save under", "database/",
...
void CActionManager::refreshRoom()
{
    ...
    if (Map.selections.isEmpty() == true) {
        if (stacker.amount() != 1) {
            QMessageBox::critical(parent, "Pandora", QString("You have to be in sync or select just one room!"));
            return;
        }
        r = stacker.first();
    } else {
        if (Map.selections.size() != 1) {
            QMessageBox::critical(parent, "Pandora", QString("You have to select just one room!"));
            return;
        }
...
{
    QString s =
        QFileDialog::getSaveFileName(parent, "Choose a filename to save under", "configs/", "XML config files (*.xml)");
    if (s.isEmpty())
        return;
...
{
    QString s = QFileDialog::getOpenFileName(parent, "Choose another configuration file to load", "configs/",
                                             "XML config files (*.xml)");
    if (s.isEmpty())
        return;

    conf->loadConfig("", s.toLocal8Bit());
...
void CActionManager::emulation_mode()
{
    if (emulationAct->isChecked()) {
        ...
        if (mappingAct->isChecked()) {
            emulationAct->setChecked(false);
            QMessageBox::critical(parent, "Pandora", QString("You have to disconnect from the game first!"));
            return;
        }
