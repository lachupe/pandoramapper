...
CGroupCommunicator::CGroupCommunicator(int _type, QObject *parent) : QObject(parent)
{
    type = Off;
...
void CGroupCommunicator::errorInConnection(CGroupClient *connection)
{
    ...
    case QAbstractSocket::ConnectionRefusedError:
        str = QString("Connection to %1:%2 refused").arg(connection->peerName()).arg(conf->getGroupManagerRemotePort());
        getGroup()->connectionRefused(str.toLocal8Bit());
        changeType(Off);
        break;
...
QByteArray CGroupCommunicator::formMessageBlock(int message, QDomNode data)
{
    ...
    block = doc.toString().toLocal8Bit();
...
void CGroupCommunicator::incomingData(CGroupClient *conn, QByteArray buff)
{
    ...
    print_debug(DEBUG_GROUP, "Raw data received: %s", (const char *)data.toLocal8Bit());

    QString error;
    int errLine, errColumn;
    QDomDocument doc("datagram");
    if (!doc.setContent(data, &error, &errLine, &errColumn)) {
        print_debug(DEBUG_GROUP, "Failed to parse the datagram. Error in line %i, column %i: %s", errLine, errColumn,
                    (const char *)error.toLocal8Bit());
        return;
    }
    ...
    while (!n.isNull()) {
        ...
        print_debug(DEBUG_GROUP, "Cycle Element name: %s", (const char *)e.nodeName().toLocal8Bit());
...
void CGroupCommunicator::reconnect()
{
    ...
    if (type == Client) {
        getGroup()->sendLog(QString("Trying to reconnect to group manager server %1:%2")
        ...
        getGroup()->sendLog(
            QString("Re-Starting group manager server listening on port %1").arg(conf->getGroupManagerLocalPort()));
...
void CGroupCommunicator::changeType(int newState)
{
    ...
    case Server:
        getGroup()->sendLog(
            QString("Starting group manager server listening on port %1").arg(conf->getGroupManagerLocalPort()));
        peer = new CGroupServer(conf->getGroupManagerLocalPort(), this);
        break;
    case Client:
        getGroup()->sendLog(QString("Trying to connect to group manager server %1:%2")
                                .arg((const char *)conf->getGroupManagerHost())
                                .arg(conf->getGroupManagerRemotePort()));
        peer = new CGroupClient(conf->getGroupManagerHost(), conf->getGroupManagerRemotePort(), this);
        break;
