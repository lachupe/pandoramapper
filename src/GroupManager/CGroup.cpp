...
bool CGroup::addChar(QDomNode node)
{
    ...
    if (isNamePresent(newChar->getName()) == true || newChar->getName() == "") {
        ...
        sendLog(
            QString("Failed to add new char, the name %1 is already present").arg((const char *)newChar->getName()));

        delete newChar;
        return false;
    } else {
        print_debug(DEBUG_GROUP, "Added new char. Name %s", (const char *)newChar->getName());
...
void CGroup::gotKicked(QDomNode message)
{
    ...
    print_debug(DEBUG_GROUP, "Kicked from group manager! Reason: %s", (const char *)text.text().toLocal8Bit());

    send_to_user("--[ You were kicked from group manager! Reason: %s", (const char *)text.text().toLocal8Bit());
    send_to_user("\r\n");
    send_prompt();
...
void CGroup::gTellArrived(QDomNode node)
{
    ...
    QDomElement text = e.toElement();
    print_debug(DEBUG_GROUP, "GTell from %s, Arrived : %s", (const char *)from.toLocal8Bit(),
                (const char *)text.text().toLocal8Bit());
    send_to_user("\r\n%s tells you [GT] '%s'.\r\n", (const char *)from.toLocal8Bit(),
                 (const char *)text.text().toLocal8Bit());
    send_to_user("\r\n");
    send_prompt();
}
...
void CGroup::renameChar(QDomNode blob)
{
    ...
    print_debug(DEBUG_GROUP, "Renaming a char from %s to %s", (const char *)oldname.toLocal8Bit(),
                (const char *)newname.toLocal8Bit());
...
void CGroup::parseScoreInformation(QByteArray score)
{
    ...
    if (score.contains("mana, ") == true) {
        ...
        self->setScore(list[0].toInt(), list[1].toInt(), list[2].toInt(), list[3].toInt(), list[4].toInt(),
                       list[5].toInt());
        ...
        QString temp = score;
        QStringList list = temp.split('/');
        self->setScore(list[0].toInt(), list[1].toInt(), 0, 0, list[2].toInt(), list[3].toInt());
